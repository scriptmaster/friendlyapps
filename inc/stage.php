...
if ( $accessedFromAppsURL ) {
	if (!$user) {
		...
		$destinations = implode( '|', array_keys( $locations ) );

		$url = 'http://maps.googleapis.com/maps/api/distancematrix/json?origins='.$my_location.'&amp;destinations='.$destinations.'&amp;sensor=false';
